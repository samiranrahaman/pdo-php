...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('EmailSettings',array('order_by'=>'EmailSettingsId DESC'));
...
if(!empty($result)){ $count = 0; foreach($result as $res){ $count++;?>
    <?php
        ...
        <tr>
			...
			if($res["IsActive"] == 1){
				echo '				
					<td><label id="'.$res["EmailSettingsId"].'" class="switch">
					<input id="'.$res["EmailSettingsId"].'" onclick="IsActive('.$res["EmailSettingsId"].')" checked class="IsActive'.$res["EmailSettingsId"].'" type="checkbox">
					<span class="slider round"></span>
					</label></td>';
			}else{
				echo '				
				<td><label id="'.$res["EmailSettingsId"].'" class="switch">
					<input id="'.$res["EmailSettingsId"].'" onclick="IsActive('.$res["EmailSettingsId"].')" type="checkbox" class="IsActive'.$res["EmailSettingsId"].'">
					<span class="slider round"></span>
					</label></td>';
			}
			echo ' 
			<td><button id="'.$res["EmailSettingsId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["EmailSettingsId"].'">Delete</button></td>
		</tr>';
	}
}else{ ?>
