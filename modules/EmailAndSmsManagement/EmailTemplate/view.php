...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('EmailTemplate',array('order_by'=>'EmailTemplateId DESC'));
    ...
    <?php
	   ...
	   if(!$AdminUserU['UserName']=='Admin'){
        ...
        <tr>
			...
			<td>'.$res["EmailTemplateName"].'</td>
			<td>'.$res["EmailTemplateSubject"].'</td>
			<td>'.$res["EmailTemplateDescription"].'</td>
			...
			if($res["IsActive"] == 1){
				echo '				
					<td><label id="'.$res["EmailTemplateId"].'" class="switch">
					<input id="'.$res["EmailTemplateId"].'" onclick="IsActive('.$res["EmailTemplateId"].')" checked class="IsActive'.$res["EmailTemplateId"].'" type="checkbox">
					<span class="slider round"></span>
					</label></td>';
			}else{
				echo '				
				<td><label id="'.$res["EmailTemplateId"].'" class="switch">
					<input id="'.$res["EmailTemplateId"].'" onclick="IsActive('.$res["EmailTemplateId"].')" type="checkbox" class="IsActive'.$res["EmailTemplateId"].'">
					<span class="slider round"></span>
					</label></td>';
			}
			echo ' 
			<td><button id="'.$res["EmailTemplateId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["EmailTemplateId"].'">Delete</button></td>
		</tr>';
	}
