...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('SMSSettings',array('order_by'=>'SMSSettingsId DESC'));
...
if(!empty($result)){ $count = 0; foreach($result as $res){ $count++;?>
    <?php
        ...
        <tr>
			...
			if($res["IsActive"] == 1){
				echo '				
					<td><label id="'.$res["SMSSettingsId"].'" class="switch">
					<input id="'.$res["SMSSettingsId"].'" onclick="IsActive('.$res["SMSSettingsId"].')" checked class="IsActive'.$res["SMSSettingsId"].'" type="checkbox">
					<span class="slider round"></span>
					</label></td>';
			}else{
				echo '				
				<td><label id="'.$res["SMSSettingsId"].'" class="switch">
					<input id="'.$res["SMSSettingsId"].'" onclick="IsActive('.$res["SMSSettingsId"].')" type="checkbox" class="IsActive'.$res["SMSSettingsId"].'">
					<span class="slider round"></span>
					</label></td>';
			}
			echo '
			<td><button id="'.$res["SMSSettingsId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["SMSSettingsId"].'">Delete</button></td>
		</tr>';
	}
}else{ ?>
