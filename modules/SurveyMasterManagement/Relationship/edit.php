...
$id = $_POST['id'];
	...
	$result = $db->getRows('Relationship',array('where'=>array('RelationshipId'=>$id),'return_type'=>'single'));
	...
	if(!empty($result)){ 
        ...
        <div class="form-group col-lg-3">
            ...
            <label>Relationship Name</label><br/>
				<input type="text" style="width: 90%;" name="URelationshipName" id="URelationshipName" value="<?php echo $result['RelationshipName']; ?>" class="form-control" required />
			</div>
			
			<div class="form-group col-lg-3">
            <input type="text" id='uaction_type' name="uaction_type" value="edit" hidden/> 
            <label>Action</label><br/>
			<button type="button" class="btn btn-primary update" id="<?php echo $result['RelationshipId']; ?>" name="update">Update Record</button>
...
<script type="text/javascript">
	$('.update').click(function() {
		var id = $(this).attr('id');
        var URelationshipName = $('#URelationshipName').val();
        ...
        $.ajax({
            url: "update.php",
            type: "POST",
            data: { id: id, URelationshipName : URelationshipName, action_type: action_type },
            success: function(data, status, xhr) {
                $('#URelationshipName').val('');
                //alert(data);
                if(data == 'success'){
