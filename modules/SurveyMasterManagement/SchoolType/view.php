...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('SchoolType',array('order_by'=>'SchoolTypeId DESC'));
...
if(!empty($result)){ $count = 0; foreach($result as $res){ $count++;?>
    <?php
        ...
        <tr>
            <td>'.$count.'</td>
			<td>'.$res["SchoolTypeName"].'</td>
			<td>'.$res["CreatedDate"].'</td>
			<td>'.$res["CreatedBy"].'</td>
            <td>'.$res["UpdatedDate"].'</td>
            <td>'.$res["UpdatedBy"].'</td>
            <td>'.$res["IsActive"].'</td>
			<td><button id="'.$res["SchoolTypeId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["SchoolTypeId"].'">Delete</button></td>
		</tr>';
	}
}else{ ?>
