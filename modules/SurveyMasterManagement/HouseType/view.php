...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('HouseType',array('order_by'=>'HouseTypeId DESC'));
...
if(!empty($result)){ $count = 0; foreach($result as $res){ $count++;?>
    <?php
        ...
        <tr>
            <td>'.$count.'</td>
			<td>'.$res["HouseTypeName"].'</td>
			<td>'.$res["CreatedDate"].'</td>
			<td>'.$res["CreatedBy"].'</td>
            <td>'.$res["UpdatedDate"].'</td>
            <td>'.$res["UpdatedBy"].'</td>
            <td>'.$res["IsActive"].'</td>
			<td><button id="'.$res["HouseTypeId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["HouseTypeId"].'">Delete</button></td>
		</tr>';
	}
}else{ ?>
