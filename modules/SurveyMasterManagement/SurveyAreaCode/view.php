...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('SurveyAreaCode',array('order_by'=>'SurveyAreaCodeId DESC'));
	...
	?>
			...
			<tr>
			<th>SL</th>
			<th>Survey Area Code</th>
			<th>Survey Area Name</th>
			<th>Branch</th>
			<th>City</th>
			<th>State</th>
			<th>Country</th>
...
if(!empty($result)){ $count = 0; foreach($result as $res){ $count++;?>
    <?php
        ...
        $CityName = $db->getRows('City',array('where'=>array('CityId'=>$res["CityId"]),'return_type'=>'single'));
		$StateName = $db->getRows('State',array('where'=>array('StateId'=>$res["StateId"]),'return_type'=>'single'));
		$CountryName = $db->getRows('Country',array('where'=>array('CountryId'=>$res["CountryId"]),'return_type'=>'single'));
		
		echo '
        <tr>
            <td>'.$count.'</td>
			<td>'.$res["SurveyAreaCode"].'</td>
			<td>'.$res["SurveyAreaName"].'</td>
            ...
            <td>'.$res["UpdatedBy"].'</td>
            <td>'.$res["IsActive"].'</td>
			<td><button id="'.$res["SurveyAreaCodeId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["SurveyAreaCodeId"].'">Delete</button></td>
		</tr>';
	}
}else{ ?>
