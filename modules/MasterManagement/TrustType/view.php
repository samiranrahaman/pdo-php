...
include_once $serverRoot.'/SBM/includes/logics/Crud.php';
    ...
    $result = $db->getRows('TrustType',array('order_by'=>'TrustTypeId DESC'));
...
if(!empty($result)){ $count = 0; foreach($result as $res){ $count++;?>
    <?php
        ...
        <tr>
            <td>'.$count.'</td>
			<td>'.$res["TrustTypeName"].'</td>
			<td>'.$res["CreatedDate"].'</td>
			<td>'.$res["CreatedBy"].'</td>
            <td>'.$res["UpdatedDate"].'</td>
            <td>'.$res["UpdatedBy"].'</td>
            <td>'.$res["IsActive"].'</td>
			<td><button id="'.$res["TrustTypeId"].'" class="edit btn btn-info">Edit</button> <button class="del btn btn-danger" id="'.$res["TrustTypeId"].'">Delete</button></td>
		</tr>';
	}
}else{ ?>
